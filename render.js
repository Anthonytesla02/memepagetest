...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const doc = await db.collection("memePages").doc(hash).get();
         ...
         const htmlFileRef = storageRef.child(`memePages/${hash}.html`); // Use the same documentId for storage location
          const url = await htmlFileRef.getDownloadURL();
        const response = await fetch(url);
        const html = await response.text();

        landingPage.innerHTML = html;
